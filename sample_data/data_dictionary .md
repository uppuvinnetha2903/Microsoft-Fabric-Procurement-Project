...
- `Azure_Blob_Source/` — master/reference data, loaded via a OneLake shortcut to Azure Blob Storage
- `SQL_Source/` — transactional data, loaded via Fabric Data Pipelines from the on-prem SQL Server

> **Note on file format:** in the live pipeline, `SQL_Source` data is read directly from SQL Server tables via a Fabric Data Pipeline database connection — never from a file. The `.csv` files here are a static extract of that same data, exported purely so this sample dataset can be shared in a public repository and the notebooks can be run without requiring a live database connection.
